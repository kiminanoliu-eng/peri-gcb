...
```python
Agent(
  description="搜索产品YouTube视频",
  prompt=f"""
  在PERI官方YouTube频道搜索 {产品名} 的产品介绍视频。
...
**重要**：
- 尝试5次后未找到 → 使用空字符串 `""`
- 页面会自动隐藏YouTube组件（rebuild_site_v2.py已实现）
...
**推荐方法**：使用Agent工具自动搜索YouTube视频

```
使用Agent工具，提示词：
"在PERI官方YouTube频道搜索 {产品名} 的产品介绍视频。
要求：英语、≤10分钟、产品介绍（不是项目案例）。
搜索关键词尝试：'{产品名}'、'{产品名} formwork'、'{产品名} system'
如果找到符合条件的视频，返回11位视频ID。
如果尝试5次后仍未找到，返回空字符串。"
```

**搜索策略**：
1. 第1次尝试：完整产品名（如"RAPID column formwork"）
2. 第2次尝试：简化产品名（如"RAPID"）
3. 第3次尝试：产品名 + "formwork"
4. 第4次尝试：产品名 + "system"
5. 第5次尝试：核心关键词
...
### 原则3: 自动化YouTube搜索
- ✅ 使用Agent工具自动搜索YouTube视频
- ✅ 尝试5次不同的搜索关键词
- ✅ 5次后未找到 → 使用空字符串 `""`
- ✅ 页面自动隐藏空组件（PDF、YouTube、项目案例）

### 原则4: 页面组件条件显示
- ✅ rebuild_site_v2.py已实现条件隐藏逻辑：
  - 如果`youtube_video_id == ""`，隐藏YouTube组件
  - 如果`pdf_link == ""`或`None`，隐藏PDF按钮
  - 如果`projects == []`，隐藏项目案例区域
- ✅ 宁可没有，也不用错误的内容
